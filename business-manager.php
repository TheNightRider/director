...
/* Registring taxonomy, can have several extra fields which can be seen in wordpress codex */
...
function business_manager_add_meta(){
    add_meta_box("business-meta", "Business Options", "business_manager_meta_options", "business", "normal", "high");
}
/* Adding business_manager_meta_options() so it can populate the box */
function business_manager_meta_options(){
    global $post;
    if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) 
        return $post_id;
    $custom = get_post_custom($post->ID);
    $address = $custom["adress"][0];
...
/* Improving display information in wordpress */
add_filter("manage_edit-business_columns", "business_manager_edit_columns");
